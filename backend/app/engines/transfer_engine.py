...
class TransferEngine:
    def analyze_transfers(self, journey: Journey) -> list[TransferDifficultyResult]:
        ...
        for i in range(len(journey.segments) - 1):
            ...
            if (
                current_seg.segment_type != TransportType.WALK
                and next_seg.segment_type != TransportType.WALK
            ):
                buffer_minutes = int(
                    (next_seg.departure_time - current_seg.arrival_time).total_seconds()
                    / 60
                )
            ...
            # Transfer involving a walk
                ...
                if i + 2 < len(journey.segments):
                    ...
                    buffer_minutes = int(
                        (
                            target_seg.departure_time - current_seg.arrival_time
                        ).total_seconds()
                        / 60
                    )
                else:
