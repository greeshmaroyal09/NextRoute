...
class RecommendationEngine:
    def tag_recommendations(self, journeys: list[ExplainedJourney]) -> list[ExplainedJourney]:
        if not journeys:
            return []
        ...
        # Fastest
        fastest = min(journeys, key=lambda j: j.scored_journey.journey.total_duration_minutes)
        ...
        if best_for_women and best_for_women.scored_journey.factor_scores.get("safety", 0.0) > 0.7:
