...
from app.domain.interfaces.repositories import (
    IBusRouteRepository,
    ITrainRouteRepository,
)
from app.infrastructure.database.models import BusRoute as BusRouteModel
from app.infrastructure.database.models import (
    Station,
    TrainStop,
)
from app.infrastructure.database.models import TrainRoute as TrainRouteModel
...
class TrainRouteRepository(ITrainRouteRepository):
    ...
    async def get_by_number(self, train_number: str) -> TrainRoute | None:
        stmt = select(TrainRouteModel).where(
            TrainRouteModel.train_number == train_number
        )
        result = await self.session.execute(stmt)
    ...
    async def list_routes_through_station(self, station_code: str) -> list[TrainRoute]:
        stmt = (
            select(TrainRouteModel)
            .join(TrainStop, TrainRouteModel.id == TrainStop.train_route_id)
            .join(Station, TrainStop.station_id == Station.id)
