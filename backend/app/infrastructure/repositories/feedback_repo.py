...
class FeedbackRepository(IFeedbackRepository):
    ...
    async def get_stats_by_route_hash(self, route_hash: str) -> Any:
        stmt = select(
            ...
            func.avg(JourneyFeedback.would_recommend.cast(func.integer)).label("recommendation_rate")
        ).where(JourneyFeedback.route_hash == route_hash)
        result = await self.session.execute(stmt)
        row = result.one_or_none()
        if not row:
            return {"count": 0, "recommendation_rate": 0.0}
        return {"count": row.count, "recommendation_rate": float(row.recommendation_rate or 0.0)}
