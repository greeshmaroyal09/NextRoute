...
ROUTE_ENGINE_CONTENT = '''from __future__ import annotations
import uuid
...
class RouteEngine:
    ...
    def find_routes(self, from_code: str, to_code: str, travel_date: datetime, max_transfers: int | None = None, allowed_modes: list[str] | None = None) -> list[Journey]:
        ...
        for src in src_nodes:
            for dst in dst_nodes:
                try:
                    paths = list(nx.shortest_simple_paths(self._graph, src, dst, weight="duration"))
    ...
    def _validate_timing(self, journey: Journey) -> bool:
        for i in range(len(journey.segments) - 1):
            ...
            if gap < self._settings.MIN_TRANSFER_BUFFER_MINS and gap > -1400: 
...
def write_engines():
    with open('app/engines/route_engine.py', 'w', encoding='utf-8') as f:
        f.write(ROUTE_ENGINE_CONTENT)
    print("Refactored RouteEngine applied.")
